...
from modelfast import GPTConfig, GPT
...
# optimizer
optimizer, grads_ma, grads_ema = model.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type)
if init_from == 'resume':
...
running_mfu = -1.0
grads_ma = {n: deque(maxlen=100) for n, p in model.named_parameters() if p.requires_grad}
grads_ema = {n: torch.zeros_like(p.data) for n, p in model.named_parameters() if p.requires_grad}
while True:
    ...
    # Forward and backward pass with gradient accumulation and GrokFast
    for micro_step in range(gradient_accumulation_steps):
        # Handle Distributed Data Parallel (DDP) synchronization
        if ddp:
        ...
        # Apply GrokFast at the end of gradient accumulation
        if micro_step == gradient_accumulation_steps - 1:
            scaler.unscale_(optimizer)  # Unscale gradients before applying GrokFast
            grads_ma = gradfilter_ma(model, grads_ma)
            grads_ema = gradfilter_ema(model, grads_ema)
    ...
    if iter_num % log_interval == 0 and master_process:
        # Calculate true loss by undoing gradient accumulation scaling
